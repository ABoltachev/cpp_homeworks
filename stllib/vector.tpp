...
template <typename T>
void Vector<T>::checkSize(size_t size) const
{
    if (size == 0) {
        throw std::logic_error("Size of vector can not be zero!");
    }

    if (size < 0) {
        throw std::logic_error("Size of vector can not be a negative number!");
    }
}
